...
namespace RJW_Genes
{
    ...
    public static class Patch_LevelledTransformation
    {
        ...
        private static void MinorChange(Pawn pawn, GeneAlteringExtension geneAlteringExtension)
        {
            ...
            if (chosen == null)
            {
                ModLog.Warning($"Error in retrieving a minor transformation gene for transforming {pawn}");
                return;
            }
            ...
            if (!pawn.genes.GenesListForReading.Any(p => p.def == chosen))
            {
                ...
                ModLog.Debug($"{pawn} experienced a minor transformation; {pawn} got new gene {chosen}.");
                pawn.genes.AddGene(chosen, !RJW_Genes_Settings.rjw_genes_genetic_disease_as_endogenes);
            }

            else
            {
                ModLog.Debug($"Tried to do a minor transformation for {pawn} - {pawn} already had {chosen} or has immunity against it");
            }
        }

        private static void MajorChange(Pawn pawn, GeneAlteringExtension geneAlteringExtension)
        {
            ...
            if (chosen == null)
            {
                ModLog.Warning($"Error in retrieving a major transformation gene for transforming {pawn}");
                return;
            }
            ...
            if (!pawn.genes.GenesListForReading.Any(p => p.def == chosen))
            {
                ...
                ModLog.Debug($"{pawn} experienced a major transformation; {pawn} got new gene {chosen}.");
                pawn.genes.AddGene(chosen, !RJW_Genes_Settings.rjw_genes_genetic_disease_as_endogenes);
            }

            else
            {
                ModLog.Debug($"Tried to do a major transformation for {pawn} - {pawn} already had {chosen}");
                ModLog.Debug($"Trying minor transformation for {pawn} instead ...");
                MinorChange(pawn, geneAlteringExtension);
            }
        }
    }
